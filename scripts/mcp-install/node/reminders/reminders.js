 ...
 */
export async function getRemindersLists() {
    try {
 ...
 */
export async function getRemindersFromList(listName) {
    try {
        ...
        const reminderItems = await reminders.getReminders(targetList.id, ['name', 'completed', 'dueDate', 'priority', 'body']);
 ...
 */
export async function createReminder(listName, title, dueDate, notes) {
    try {
        // First get the list ID by name
        const lists = await reminders.getLists();
        const targetList = lists.find(list => list.name === listName);
 ...
 */
export async function completeReminder(listName, reminderName) {
    try {
        // First get the list ID by name
        const lists = await reminders.getLists();
        const targetList = lists.find(list => list.name === listName);
        ...
        const reminderItems = await reminders.getReminders(targetList.id, ['name', 'id']);
 ...
 * Delete a reminder
 */
export async function deleteReminder(listName, reminderName) {
    try {
        // First get the list ID by name
        const lists = await reminders.getLists();
        const targetList = lists.find(list => list.name === listName);
